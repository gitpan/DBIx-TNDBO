...
you intent to interact with. Otherwise, give the name of the key in the
...
Create a record a new record object.

  my $word_dbo = YDB();

This creates an object with schema relevant getters &amp; setters. In this
example we've created an instance of the `word` table in the `language` table.
...
abreviated syntax for different record object constructor types and the
get/set operations on record objects.
...
the same name. For example:
  ...
  use YDB qw( db_a db_b );

  my $word_dbo = YDB();

If there is both a db_a.word table and a db_b.word table then the results in
this case are not defined.
